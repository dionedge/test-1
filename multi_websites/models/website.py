...
MULTI_FOOTER = TECHNICAL_NAME + ".multi_website_footer_%d"
...
class Website(models.Model):
	...
	wk_website_footer_id = fields.Many2one(
		comodel_name="ir.ui.view", 
		domain=[("type","=","qweb")],
		string="Website Footer",
	)
	...
	def create(self, vals):
		...
		result.activateCustomFooter()
		result.updateFooterView()
		return result

	def write(self, vals):
		result = super(Website, self).write(vals)
		if 'wk_website_footer_id' in vals:
			self.activateCustomFooter()
			self.updateFooterView()
		if vals.get('website_multi_theme_id'):
			self._activate_website_multi_theme()
		return result

	def updateFooterView(self):
		string="multi_websites.multi_website_footer_%s"%self.id
		obj = self.env.ref("%s"%string)
		if not self.wk_website_footer_id:
			obj.active = False
		else:
			obj.active = True

	def activateCustomFooter(self):
		main_footer = self.env.ref("multi_websites.multi_website_footer")
		mt_footer = self.env.ref("multi_websites.multi_website_footer")
		main_views = main_footer | mt_footer
		for website in self:
			custom_footer = website._duplicate_view_for_website( mt_footer,MULTI_FOOTER % website.id, True)
			custom_footer.arch = mt_footer.arch
			custom_views = custom_footer
			...
			for origin_view in website.mapped("website_multi_theme_id.multi_theme_asset_ids.view_id"):
				...
				if copied_view.inherit_id and copied_view.inherit_id < main_views:
					...
					if copied_view.inherit_id < main_footer:
						copied_view.inherit_id = custom_footer
						data.attrib["inherit_id"] = custom_footer.key
	...
	def get_pricelist_available(self, show_visible=False):
		""" Set the pricelist according to the website in the session.
		"""
		res = super(Website, self).get_pricelist_available(show_visible)
		website = request and hasattr(request, 'website') and request.website or None
		if website.product_pricelist_ids:
			request.session['website_sale_current_pl'] = website.product_pricelist_ids[0].id
			res = website.product_pricelist_ids[0]
		return res